...
public:
	...
	template <class Type> void SetCoordinate(const Type *x, const char *c) {
		...
		else { wrong_coordinate_setting(); }
	}

	template <class Type, class Coordinate> void Transfer(Type t12[3][3], Type r12[3], Coordinate &coord1, Coordinate &coord2) {
		Type *origin1_ptr = coord1.origin;
		Type *origin2_ptr = coord2.origin;
		...
		if (coord1.base == coord2.base) {
			...
			Msolver(*coord1.Ttransf, *t12, 3, 3);
		}
		else { wrong_comp_coordinate_diff_base(); }

	}

	void SetBase(Coordinate *C) {
		base = C;
	}
};
